...
Since, both fpop and cpt.mean uses pruning methods to optimize the runtime which on average takes O(n log n) time complexity we can see that both these algorithms take less time compared to opart_gaussian which is always quadratic in time complexity.
...
fpfit <- c()
cpfit <- c()
for(penalty in 10^(-5:10)){
  timing <- microbenchmark(
    "opart"={
        opfit <- opart::opart_gaussian(selected_data$logratio, penalty)
    }, 
    "fpop"={
      if(requireNamespace("fpop", quietly = TRUE))
        fpfit <- fpop::Fpop(selected_data$logratio, penalty)
    },
    "changepoint"={
        cpfit <- changepoint::cpt.mean(data = selected_data$logratio, penalty = "Manual", 
