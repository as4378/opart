...
void InitializeCumSums(const double* data, double* sums, const int n_data){
  double total = 0;
  int x;
  for(x = 0; x < n_data; x++){
    sums[x] = 0;
...
//Calculating mean of a given segment
double SampleMean(int initial, int final, double* sums){
  double mean = 0;
...
//F(0) = B
void FindOptimalPartitions(const double* data_points, double* sums, double* dp,
  ...
  //F(1) = F(0) + Cy1:1 + B
  //F(0) = B
  dp[0] = -1*beta + GetPoissonLoss(0, 0, sums) + beta;
  positions[0] = -1;
  ...
  for(t = 1; t < n_data; t++){
    for(s = -1; s < t; s++){
      if(s == -1){
        //F(0) = B
        f_tau = -1*beta + GetPoissonLoss(s + 1, t, sums) + beta;
      }
      else{
        f_tau = dp[s] + GetPoissonLoss(s + 1, t, sums) + beta;
      }
...
int opart_poisson(const int n_data, const double *data_ptr, const double penalty,
  ...
  InitializeCumSums(data_ptr, sums, n_data);

  FindOptimalPartitions(data_ptr, sums, dp, positions, penalty, n_data);
